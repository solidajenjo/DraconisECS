...
bool Editor::init()
{
	...
	if( !loadLayout() )
	{
		// If loading fails, we already have the default theme from
		// setDefaultTheme()
		return true;
	}
...
bool Editor::saveLayout( const std::string& filename ) const
{
	...
	std::filesystem::path layoutPath =
		std::filesystem::path( basePath ) / ( filename.empty() ? "layout.json" : filename );
	SDL_free( basePath );
	...
	for( const auto& [name, state] : panelStates )
	{
		j["panels"][name] = { { "visible", state.isVisible },
							  ...
							  { "dockId", static_cast<uint32_t>( state.dockId ) } };
	}
	...
	j["theme"] = {
		...
		{ "bgColor",
		  { currentTheme.bgColor.x, currentTheme.bgColor.y, currentTheme.bgColor.z, currentTheme.bgColor.w } },
		{ "titlebarColor",
		  { currentTheme.titlebarColor.x,
			currentTheme.titlebarColor.y,
			currentTheme.titlebarColor.z,
			currentTheme.titlebarColor.w } },
		{ "titlebarActiveColor",
		  { currentTheme.titlebarActiveColor.x,
			currentTheme.titlebarActiveColor.y,
			currentTheme.titlebarActiveColor.z,
		...
		{ "accentActiveColor",
		  { currentTheme.accentActiveColor.x,
			currentTheme.accentActiveColor.y,
			currentTheme.accentActiveColor.z,
			currentTheme.accentActiveColor.w } },
		{ "buttonColor",
		  { currentTheme.buttonColor.x,
			currentTheme.buttonColor.y,
			currentTheme.buttonColor.z,
			currentTheme.buttonColor.w } },
		{ "buttonHoverColor",
		  { currentTheme.buttonHoverColor.x,
			currentTheme.buttonHoverColor.y,
			currentTheme.buttonHoverColor.z,
			currentTheme.buttonHoverColor.w } },
		{ "buttonActiveColor",
		  { currentTheme.buttonActiveColor.x,
			currentTheme.buttonActiveColor.y,
			currentTheme.buttonActiveColor.z,
...
void Editor::addSubMenuItem( const std::string& menu,
