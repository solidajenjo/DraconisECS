...
{
    // No need to create a framebuffer here as we'll use the one from render module
...
void RenderPanel::render()
{
    ...
    if (ImGui::Begin(getName(), nullptr, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse))
    {
        ...
        // Get the framebuffer texture ID from the render module's active framebuffer
        GLuint textureId = m_RenderModule->getDefaultFramebuffer()->getTextureID();
        if (textureId == 0)
        {
            std::cout << "Error: Invalid framebuffer texture ID from render module" << std::endl;
        }
        else
        {
            // Display the framebuffer texture
            ImGui::Image((void *)(intptr_t)textureId, viewportSize,
                         ImVec2(0, 1), // UV coordinates (flipped vertically for OpenGL)
                         ImVec2(1, 0));
        }
